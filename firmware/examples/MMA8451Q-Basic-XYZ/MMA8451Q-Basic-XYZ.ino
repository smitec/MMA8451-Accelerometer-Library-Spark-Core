...
#define MMA8451_ADDRESS 0x1D  // 0x1D if SA0 is high, 0x1C if low
...
{  
  ...
  for (int i = 0 ; i < 3 ; i++)
  {
    accelG[i] = (float) accelCount[i] / ((1<<14)/(2*GSCALE));  // get actual g value, this depends on scale being set
  }
...
void readAccelData(int *destination)
{
  ...
  for(int i = 0; i < 3 ; i++)
  {
    ...
    // If the number is negative, we have to make it so manually (no 14-bit data type)
    if (rawData[i*2] > 0x7F)
    {  
      gCount -= 0x4000;
    }
...
void initMMA8451()
{
  ...
  {
    Serial.print("Could not connect to MMA8451Q: 0x");
